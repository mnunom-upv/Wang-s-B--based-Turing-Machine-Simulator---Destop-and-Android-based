"""
Desktop application implementing a visual Turing Machine simulator
using PyQt6.

The application allows users to:
- Write a Turing Machine program
- Validate its syntax
- Execute it step-by-step or automatically
...
from PyQt6.QtGui import QSyntaxHighlighter, QTextOption
...
class DrawWidget(QWidget):
    """
    Custom widget responsible for rendering the Turing Machine tape.

    It draws:
    - Each tape cell as a rectangle
    - The symbol stored in each cell
    - An arrow indicating the current head position
    """
    ...
    def paintEvent(self, event):
        """
        Paint the tape and head indicator.
        ...
        qp.setPen(QPen(Qt.GlobalColor.black, 5))
...
class SyntaxHighlighter(QSyntaxHighlighter): #Codigo para resaltar de un color una linea de un PlainTextEdit
    """
    Custom syntax highlighter used to highlight
    ...
    def highlight_line(self, line, fmt):
        """
        Highlight a specific line with a given format.
    ...
    def highlightBlock(self, text):
        """
        Apply formatting to the current text block.

        Called internally by Qt.
        """
...
class MainWindow(QWidget): #Clase principal
    """
    Main application window implementing the Turing Machine simulator.
    ...
    def __init__(self, parent=None): 
        ...
        mainLayoutF = QVBoxLayout()

        upperLayout = QHBoxLayout()
        buttonImportar = QPushButton("Open")
        buttonExportar = QPushButton("Export")
        self.botonScreenshots = QPushButton("ScreenShots")
        self.Botonabout = QPushButton("About")
        upperLayout.addWidget(buttonImportar)
        upperLayout.addWidget(buttonExportar)
        upperLayout.addWidget(self.botonScreenshots)
        upperLayout.addWidget(self.Botonabout)

        mainLayoutF.addLayout(upperLayout)
        ...
        self.input.setMaximumWidth(440)
        ...
        self.buttonContinue = QPushButton("Continue")
        ...
        firstButtonLayout.addWidget(self.buttonContinue)
        firstButtonLayout.addWidget(buttonStop)
        ...
        #buttonExportarViejo = QPushButton("Save File")
        #buttonImportarViejoViejo = QPushButton("Open File")
        #layoutBotonesArchivo.addWidget(buttonImportarViejoViejo)
        #layoutBotonesArchivo.addWidget(buttonExportarViejo )
        leftLayout.addLayout(layoutBotonesArchivo)

        #self.botonScreenshots_Old = QPushButton("Take Screenshot")
        #self.botonScreenshots_Old.setCheckable(True)
        #leftLayout.addWidget(self.botonScreenshots_Old)
        ...
        #self.input.setWordWrapMode(QTextOption.NoWrap)  #Qt5 to Qt6
        ...
        mainLayoutF.addLayout(mainLayout)

        self.setLayout(mainLayoutF)
        ...
        self.input.textChanged.connect(self.limpiarInput)
    ...
    def limpiarInput(self): #Metodo que ayuda a limpiar el resaltado cuand se está editando el código.
        """
        Handle text edits in the program editor.

        Updates line numbers and resets execution state
        when the program text changes.
        """
        X=1
        ...
        self.update_line_numbers()
    #def Fake_limpiarInput(self): #Metodo que ayuda a limpiar el resaltado cuand se está editando el código.
        try:
    ...
    def continuarPrograma(self):#Continuar con la ejecucion del programa
        """
        Resume program execution after a pause.
        """
    ...
    def scrollEditText(self,value): #Asignar el valor del scroll al otro scroll
        """
        Synchronize vertical scrolling between editors.
        ...
        print ("Valor del Scroll",value)
        
    def copyCursorPosition(self):
        """
        Synchronize cursor position between
        the code editor and the line-number editor.
        """
    ...
    def guardarImagen(self):
        """
        Capture and save a screenshot of the application window.
        """
    ...
    def update_line_numbers(self): #Actualizar el numero de lineas que hay en el codigo
        """
        Update the line number display to match the editor content.
        """
    ...
    def cargarPrograma(self): #funcion para inicializar los valores del programa
        """
        Load, preprocess, and validate the Turing Machine program.

        This method:
        - Cleans the input
        - Tokenizes instructions
        - Validates syntax and semantics
        - Starts execution if valid
        """
    ...
    def exportarCodigo(self): #Funcion para exportar el codigo
        """
        Export the current program to a text file.
        """
    ...
    def importarCodigo(self): #Funcion para importar el codigo
        """
        Import a program from a text file.
        """
    ...
    def actualizarArregloWidget(self): #Funcion para actualizar las posiciones del widget de dibujo
        ...
        This method advances the instruction pointer,
        updates the tape, highlights the current line,
    ...
    def colocarCursor(self,paso): #Colocar cursor en cierta posicion
        cursor = self.input.textCursor()
        #cursor.movePosition(QTextCursor.StartOfLine, QTextCursor.MoveAnchor) # From QT5 to Qt6
        cursor.movePosition(QTextCursor.MoveOperation.StartOfLine, QTextCursor.MoveMode.MoveAnchor)
        for i in range(paso):
#            cursor.movePosition(QTextCursor.Down) # From QT5 to Qt6
